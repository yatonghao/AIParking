...
void easyInput::insertValue(const QString &v)
{
    ...
    }else if (currentEditType == "QTableWidget") {
        currentTextEdit->insertPlainText(currentTableWight->property("upper").toBool() ? value.toUpper() : value);
...
void easyInput::deleteValue()
{
    if (currentEditType == "QLineEdit") {
        ...
        QKeyEvent keyPress(QEvent::KeyPress, Qt::Key_Backspace, Qt::NoModifier, QString());
        QApplication::sendEvent(currentTableWight, &keyPress);
    }
}
